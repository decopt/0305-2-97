...
interface AppointmentFormProps {
  ...
  customUrl?: string; // Make customUrl optional
}
...
export const AppointmentForm = ({ 
  ...
  const handleSubmitAppointment = async (values: any) => {
    try {
      ...
      if (appointment && appointment.id) {
        // Se for edição, faz update
        ({ error } = await supabase
          .from('appointments')
          .update(appointmentData)
          .eq('id', appointment.id));
      } else {
        // Se for novo, faz insert
        ({ error } = await supabase
          .from('appointments')
  ...
  const isValidTime = (time: string) => {
    ...
    // Bloqueia apenas horários realmente passados
            ...
            <>
              ...
              {loadingHours ? (
                ...
                <Listbox value={selectedTime} onValueChange={setSelectedTime}>
                  ...
                  <ListboxContent>
                    {availableHours
                      .filter(hour => hour.is_active !== false)
                      .map((hour) => {
                        ...
                        const interval = 30; // Sempre 30 minutos
                        const timeSlots = [];
                        let currentTime = new Date(startTime);

                        while (addMinutes(currentTime, interval) <= endTime) {
